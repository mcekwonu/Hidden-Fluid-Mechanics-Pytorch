...
class CylinderResNetHFM(nn.Module):
    ...
    def data_loss(self, x, y, t, u, v):
        u_pred, v_pred, _ = self.net_uvp(x, y, t)
    ...
    def plot_predicted_params_in_time(self, params, time_select, name):
        ...
        plt.imshow(params, cmap="jet", norm=v_norm)
        if name == "vorticity":
            plt.title(r"Learned $\omega$(x, y) (t = {:.2f}s)".format(time_select))
        else:
    ...
    def make_gif(self, input_dir, start_time, end_time, interval, name, fps=5):
        save_dir = f"../logs/{self.model_name}/animation_gif"
        self.create_dir(save_dir)
    ...
    def predict_drag_lift(self, t_cyl):
        ...
        scipy.io.savemat(f"../logs/{self.model_name}/results/drag_and_lift_force.mat", {"F_D": F_D, "F_L": F_L})
...
if __name__ == "__main__":
    ...
    t_start = t_test.min()
    t_end = t_test.max()
    delta_t = t_test[1] - t_test[0]

    pinn.make_gif(input_dir=f"../logs/{pinn.model_name}/snapshots/predict_in_time",
                  start_time=t_start, end_time=t_end, interval=delta_t, name="u_compared", fps=10)
    pinn.make_gif(input_dir=f"../logs/{pinn.model_name}/snapshots/predict_in_time",
                  start_time=t_start, end_time=t_end, interval=delta_t, name="v_compared", fps=10)
    pinn.make_gif(input_dir=f"../logs/{pinn.model_name}/snapshots/p",
                  start_time=t_start, end_time=t_end, interval=delta_t, name="p", fps=10)
    pinn.make_gif(input_dir=f"../logs/{pinn.model_name}/snapshots/vorticity",
                  start_time=t_start, end_time=t_end, interval=delta_t, name="vorticity", fps=10)
    ...
    del t_test, test_data, x_train, y_train, t_train, u_train, v_train, minmax_train
